...
class Mutator:
    ...
    def __get_mutations(
        ...
        mutations = {
            ...
            for typ, start in tqdm(
                zip(
                    choice(
                        # type: ignore
                        list(rng.mutation_settings.mut_chances.keys()),
                        p=list(
                            rng.mutation_settings.mut_chances.values()),  # type: ignore
                        size=len(start_positions)),
                    start_positions),
                desc=  # type: ignore
                f"Generating mutations in range: {rng.start+1}-{rng.stop+1}",
                position=1,
        ...
        for pos in tqdm(list(mutations.keys()),
            ...
            mutations[pos] = mut
            if mut.type in [MutType.SN, MutType.MN, MutType.IN]:
    ...
    def __get_stop_position(mutation: Mutation, mut_lengs: dict[str,
        ...
        elif mutation.type is MutType.MN:
            mutation.stop = randint(
        ...
        while pos < len(sequence):
            if pos in muts:
                if muts[pos].type is MutType.SN:
                    record = VcfRecord("sn", pos + 1, ref=self.__convert_ambiguous(sequence[pos]))
                ...
                elif muts[pos].type is MutType.MN:
                    length = muts[pos].stop + 1 - pos
                    record = VcfRecord("mn", pos + 1, ref=self.__convert_ambiguous(sequence[pos: pos + length]))
                    record.alt = self.__get_insert(length)
                    record.len = length
                    record.end = pos + length
                    pos = muts[pos].stop
                    ...
                    if pos > 0:
                        record.ref = self.__convert_ambiguous(sequence[pos - 1])

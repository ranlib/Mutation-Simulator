...
def get_args() -> "Namespace":
    ...
    parser = ArgumentParser(
        prog="mutation-simulator",
        description="See https://github.com/mkpython3/Mutation-Simulator for more information about this program.",
    )
    parser.add_argument("infile", type=Path, help="Path of the reference Fasta file")
    parser.add_argument(
        "-o",
        "--output",
        type=Path,
    ...
    parser.add_argument(
        "-v", "--version", action="version", version=f"Mutation-Simulator {__version__}"
    )

    subparsers = parser.add_subparsers(
        dest="mode",
        help="Generate mutations or interchromosomal translocations via RMT or arguments",
    )
    subparsers.required = True

    parser_args = subparsers.add_parser(
        "args", help="Use commandline arguments for mutations instead of RMT"
    )
    ...
    parser_args.add_argument(
        "-snb",
        "--snpblock",
        help=f"Amount of bases blocked after SNP. Default = {Defaults.BLOCK}",
        type=int,
    ...
    parser_args.add_argument(
        "-mn",
        "--mnp",
        help=f"MNP rate. Default = {Defaults.RATE}",
        type=float,
        default=Defaults.RATE,
    )
    parser_args.add_argument(
        "-mnb",
        "--mnpblock",
        help=f"Amount of bases blocked after MNP. Default = {Defaults.BLOCK}",
        type=int,
        default=Defaults.BLOCK,
    )
    parser_args.add_argument(
        "-mnpmin",
        "--mnpminlength",
        help=f"Minimum length of mnp. Default = {Defaults.MINLEN}",
        type=int,
        default=Defaults.MINLEN+1,
    )
    parser_args.add_argument(
        "-mnpmax",
        "--mnpmaxlength",
        help=f"Minimum length of mnp. Default = {Defaults.MINLEN}",
        type=int,
        default=Defaults.MAXLEN+1,
    )
    parser_args.add_argument(
        "-titv",
        "--transitionstransversions",
        help=f"Ratio of transitions:transversions likelihood. Default = {Defaults.TITV}",
        type=float,
    ...
    parser_args.add_argument(
        "-inb",
        "--insertblock",
        help=f"Amount of bases blocked after insert. Default = {Defaults.BLOCK}",
        type=int,
        default=Defaults.BLOCK,
    )
    parser_args.add_argument(
    ...
    parser_args.add_argument(
        "-deb",
        "--deletionblock",
        help=f"Amount of bases blocked after deletion. Default = {Defaults.BLOCK}",
        type=int,
        default=Defaults.BLOCK,
    )
    parser_args.add_argument(
    ...
    parser_args.add_argument(
        "-ivb",
        "--inversionblock",
        help=f"Amount of bases blocked after inversion. Default = {Defaults.BLOCK}",
        type=int,
        default=Defaults.BLOCK,
    )
    parser_args.add_argument(
    ...
    parser_args.add_argument(
        "-dub",
        "--duplicationblock",
        help=f"Amount of bases blocked after duplication. Default = {Defaults.BLOCK}",
        type=int,
        default=Defaults.BLOCK,
    )
    parser_args.add_argument(
    ...
    parser_args.add_argument(
        ...
        help=f"Minimum length of translocations. Default = {Defaults.MINLEN}",
        type=int,
        default=Defaults.MINLEN,
    )
    parser_args.add_argument(
        ...
        help=f"Maximum length of translocations. Default = {Defaults.MAXLEN}",
        type=int,
        default=Defaults.MAXLEN,
    )
    parser_args.add_argument(
        "-tlb",
        "--translocationblock",
        help=f"Amount of bases blocked after translocations. Default = {Defaults.BLOCK}",
        type=int,
        default=Defaults.BLOCK,
    )
    parser_args.add_argument(
        "-a",
        "--assembly",
        help=f"Assembly name for the VCF file. Default = '{Defaults.ASSEMBLY_NAME}'",
        default=Defaults.ASSEMBLY_NAME,
    )
    parser_args.add_argument(
        "-s",
        "--species",
        help=f"Species name for the VCF file. Default = '{Defaults.SPECIES_NAME}'",
        default=Defaults.SPECIES_NAME,
    )
    parser_args.add_argument(
        "-n",
        "--sample",
        help=f"Sample name for the VCF file. Default = '{Defaults.SAMPLE_NAME}'",
        default=Defaults.SAMPLE_NAME,
    )

    parser_it = subparsers.add_parser(
        "it", help="Generate interchromosomal translocations via the command line"
    )
    parser_it.add_argument(
        "interchromosomalrate",
        help="Rate of interchromosomal translocations",
        type=float,
    )

    parser_rmt = subparsers.add_parser(
        "rmt", help="Use random mutation table instead of arguments"
    )
